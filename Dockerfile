...
MAINTAINER Davide Setti <mattia@larentis.eu>
...
RUN rm -Rf /var/lib/pgsql/9.3/data &&
  su -c "/usr/pgsql-9.3/bin/initdb -D /var/lib/pgsql/9.3/data --encoding=utf8" postgres

USER postgres
RUN /usr/pgsql-9.3/bin/pg_ctl -D /var/lib/pgsql/9.3/data start &&
  sleep 1 &&
  createuser -d -s -r -l docker &&
  psql -c "ALTER USER docker WITH ENCRYPTED PASSWORD 'docker'" &&
